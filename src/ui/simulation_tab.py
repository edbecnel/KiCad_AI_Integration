"""Embedded Simulation tab for the Assistant shell."""
...
class SimulationTab(AssistantTabPanel):
    """Hosts SimulationShell inline."""

    HELP_TOPIC_ID = "simulation"

    def __init__(self, parent: wx.Window) -> None:
        if wx is None:
            raise RuntimeError("wxPython is required for SimulationTab")
        super().__init__(parent)
        self._loaded_path: Path | None = None
        self._shell: SimulationShell | None = None

        self._placeholder = wx.StaticText(
            self,
            label="Select a project and click Refresh context to load simulation gaps.",
        )
    ...
    def on_context_refreshed(self, ctx: ProjectContext, summary: str) -> None:
        ...
        self._shell = SimulationShell(self._shell_slot, new_path, embedded=True)
