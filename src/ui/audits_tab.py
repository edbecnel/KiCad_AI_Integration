"""Embedded Audits tab for the Assistant shell."""
...
from ui.audits_shell import AuditsShell
...
class AuditsTab(AssistantTabPanel):
    """Hosts AuditsShell inline."""

    HELP_TOPIC_ID = "audits"

    def __init__(self, parent: wx.Window) -> None:
        if wx is None:
            raise RuntimeError("wxPython is required for AuditsTab")
        super().__init__(parent)
        self._loaded_path: Path | None = None
        self._shell: AuditsShell | None = None

        self._placeholder = wx.StaticText(
            self,
            label="Select a project and click Refresh context to run one-click audits.",
        )
    ...
    def on_context_refreshed(self, ctx: ProjectContext, summary: str) -> None:
        ...
        self._shell = AuditsShell(self._shell_slot, new_path, embedded=True)
